...
// Importa o caderno 2024 somente com chave administrativa e liberação temporária.
router.post('/admin/importar-enem-2024', async (req, res) => {
    ...
    try {
        ...
        if (erros.length) return res.status(400).json({ erro: 'Caderno 2024 reprovado na validação.', erros });

        const normalizadas = dados.questoes.map(normalizarQuestao);
        for (const questao of normalizadas) {
            const erro = erroValidacao(questao);
            if (erro) return res.status(400).json({ erro: `Questão ${questao.id}: ${erro}` });
        }
        ...
        res.json({ sucesso: true, recebidas: normalizadas.length,
            inseridas: resultado.upsertedCount, atualizadas: resultado.modifiedCount,
            total2024, anuladas: [129], mensagem: 'Caderno ENEM 2024 importado.' });
    } catch (err) {
