...
1. Extrair os dois PDFs em registros com os mesmos campos de `enem-2025-azul.json`, acrescentando `origem: "ENEM_OFICIAL"` e `fonte_url` da prova respectiva.
2. Transcrever separadamente o gabarito oficial para `gabarito-2024-azul.json` no formato `{"respostas":{"1":"A", ...,"180":"E"}}`. Usar `"ANULADA"` quando indicado pelo INEP. Não inferir respostas a partir do texto da questão.
3. Guardar as imagens referenciadas em `assets/enem/2024/azul/` e revisar texto, fórmulas, alternativas e imagens página a página.
4. Após conferência independente, marcar `revisaoVisualAprovada: true` no manifesto e executar, dentro de `enemverse-backend`:
   `node scripts/validar-caderno-2024.js dados/enem-2024-azul.json dados/gabarito-2024-azul.json`
5. Importar somente no MongoDB DEV após a validação completa; conferir contagens, filtros, enunciados e respostas no Preview. A promoção para a produção exigirá uma etapa posterior.
